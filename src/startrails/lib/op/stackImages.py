...
from lib.util import applyMask, imwrite
...
class StackImages(Observable):
    ...
    def makeFadeGradient(frameCount, fadeFrameCount=None):
        if fadeFrameCount is None:
            fadeFrameCount = int(0.2*frameCount)

        fadeGradient = [1] * frameCount
        for i in range(0, fadeFrameCount):
            brightness = (i+1)/fadeFrameCount
            fadeGradient[i] = brightness
            fadeGradient[len(fadeGradient)-i-1] = brightness
